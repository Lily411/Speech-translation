...
model_dir = r'C:\Users\Lily\Desktop\python\llama\speech-to-text\zn\model'
...
common_voice["train"] = load_dataset(path='rakshya34/filtered_voice_english_v1', split="train", trust_remote_code=True)
...
small_common_voice= common_voice["train"].select(range(0, 1000))
small_common_voice= small_common_voice.rename_column('transcription', 'sentence')
print(len(small_common_voice))
#---------------------------------------------------------------------------------------------------------------------------------------
import torch
...
print(f"Base: the mean wer of English speech to text is {base_mean_wer}")
print(f"Fine tune: the mean wer of English speech to text is {peft_mean_wer}")